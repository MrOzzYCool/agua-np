...
import {
  ...
  BarChart3, ClipboardList, UserCog, LogOut, Droplets, Home,
} from "lucide-react";
...
const navItems: NavItem[] = [
  { href: "/yaku/dashboard", label: "Inicio", icon: LayoutDashboard, roles: ALL_ROLES },
  { href: "/yaku/dashboard/socios", label: "Socios", icon: Users, roles: ["tecnico", "administrador"] },
  { href: "/yaku/dashboard/pagos", label: "Pagos", icon: CreditCard, roles: ["cobrador", "administrador"] },
  { href: "/yaku/dashboard/morosidad", label: "Morosos", icon: AlertTriangle, roles: ["administrador"] },
  { href: "/yaku/dashboard/reportes", label: "Reportes", icon: BarChart3, roles: ["administrador"] },
  { href: "/yaku/dashboard/historial", label: "Historial", icon: ClipboardList, roles: ["administrador"] },
  { href: "/yaku/dashboard/usuarios", label: "Usuarios", icon: UserCog, roles: ["administrador"] },
];

type Props = { rol: string; nombreUsuario: string };

export default function YakuSidebar({ rol, nombreUsuario }: Props) {
      ...
      <div className="px-6 py-7 border-b border-slate-700/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 leading-tight">Sistema de Agua</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
        ...
        {visibleItems.map((item) => {
          ...
          const isActive = pathname === item.href ||
            (item.href !== "/yaku/dashboard" && pathname.startsWith(item.href));
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition ${
                isActive ? "bg-sky-500/15 text-sky-400" : "text-slate-400 hover:bg-slate-800 hover:text-slate-200"
              }`}>
